...
private:
  ...
  node *leftRot(node *root) { assert(false); }
  node *leftRightRot(node *root) { assert(false); }
  node *rightLeftRot(node *root) { assert(false); }
...
public:
  ...
  virtual void printInOrder() override { assert(false); }
  virtual int size() override { return this->count; }
  virtual int height() override { return height(this->root); }
};
